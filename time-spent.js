$(() => {
	...
	class TimeSpent {
		...
		constructor(time, immediate) {
			...
			/**
			 * Returns object of results calc.
			 */
			this.data = () => ({
				lgName: _lgName,
				lgAmnt: _lgAmnt,
	...
	function totalTimeSpent() {
		...
		if (totalElapsed >= 3600) {
			...
			smallerAmount = Math.round(+totalFloat.toFixed(2)
			                                      .split('.')[1] * 60 / 100);
		...
		$('#smaller-amount-name')
			.html(smallerName);
		$('#larger-amount')
			.html(Math.floor(largerAmount));
		$('#smaller-amount')
			.html(Math.floor(smallerAmount));
	}
	...
	function makeChart(data, markers) {
		...
		var x = d3.time.scale()
			...
			y = d3.scale.linear()
			      ...
			      .domain([
				      0,
				      d3.max(data, function (d) { return d.pct95; })
			      ]);
